...
# ==============================================================================
# ==============================================================================
# SECTION 1: DATA EXTRACTION ENGINE (UPDATED FOR DEDUPLICATED HASHSET)
# ==============================================================================
    ...
    for (file_path, start_line, end_line), group_edges in groups.items():
        ...
        if os.path.exists(file_path) and os.path.isfile(file_path):
            try:
                ...
                clamped_end = min(len(all_lines), end_line)
                code_text = "".join(all_lines[clamped_start - 1 : clamped_end])
                ...
                code_text = f"// Source parsing skipped for file: {file_path}"
        else:
            code_text = f"// Infrastructure Reference Segment: {file_path}"
        ...
        # FIXED: Accessing keys directly without internal double-quote escape processing
        first = group_edges[0]

        text_content = (
            f"// FILE: {file_path}\n"
            f"// CLASS: {first['source_class']}\n"
            f"// COMPONENT CONTEXT: {first['source_method']}\n"
        ...
        graph_edges = [
            {
                "source": f"{e['source_class']}.{e['source_method']}", 
                "target": f"{e['target_class']}.{e['target_method']}", 
...
class CodebaseRAGPipeline:
    ...
    def retriever(self, question: str, limit: int = 8):

        query_vector = self.embeddings_model.embed_query(
        ...
        for hit in search_results.points:
            ...
            file_name = payload["file_path"].lower()
            class_name = payload["class_name"].lower()
            method_name = payload["method_name"].lower()

            if "audit" in question_lower:
                if "audit" in file_name or "audit" in class_name:
                    score += 0.15

            if "employee" in question_lower:
                if "employee" in file_name or "employee" in class_name:
                    score += 0.10

            if "report" in question_lower:
                if "report" in method_name:
                    score += 0.10

            reranked_hits.append((score, hit))

        reranked_hits.sort(key=lambda x: x[0], reverse=True)

        search_hits = [hit for score, hit in reranked_hits]

        context_blocks = []
        seen_chunks = set()

        cypher_query = """
        MATCH (caller:Method)-[r1:INVOKES]->(node:Method {id:$node_id})
        RETURN
            'INCOMING' AS direction,
            caller.name AS related_method,
            caller.class AS related_class,
            r1.type AS relation

        UNION

        MATCH (node:Method {id:$node_id})-[r2:INVOKES]->(callee:Method)
        RETURN
            'OUTGOING' AS direction,
            callee.name AS related_method,
            callee.class AS related_class,
            r2.type AS relation
        LIMIT $limit
        """
            ...
            for hit in search_hits[:3]:
                ...
                if chunk_key not in seen_chunks:
                    context_blocks.append(
                        ...
                        f"METHOD: {payload['method_name']}\n\n"
                        f"{payload['document_text']}"
                    )
                ...
                for record in result:
                    ...
                    if record["relation"] in ignore_relations:
                        continue

                    if not record["related_class"]:
                        continue
                    ...
                    qdrant_filter = Filter(
                        must=[
                            FieldCondition(
                                key="class_name",
                                match=MatchValue(
                                    value=record["related_class"]
                                )
                            ),
                            FieldCondition(
                                key="method_name",
                                match=MatchValue(
                                    value=record["related_method"]
                                )
                            )
                        ]
                    )
                    ...
                    if points_list:
                        ...
                        connected = points_list[0]

                        graph_key = (
                            connected.payload["file_path"],
                            connected.payload["class_name"],
                            connected.payload["method_name"]
                        )

                        if graph_key not in seen_chunks:

                            context_blocks.append(
                                f"--- {record['direction']} "
                                f"({record['relation']}) ---\n"
                                f"{connected.payload['document_text']}"
                            )

                            seen_chunks.add(graph_key)
    ...
    def ask(self, question: str, generation: bool = False) -> str:
        ...
        print(self.retriever(question, limit=8))
...
Only modify code that is directly required to satisfy the request.

Do NOT modify a method simply because it appears in the retrieved context.

Before emitting a change, verify that the method's implementation must actually change.

If a retrieved method already satisfies the requirement, do not include it in the output.
6. EXISTING OWNERSHIP CHECK:

Before modifying a method, verify which class actually owns the method.

Never place a method into a class unless that method already exists there.

Example:
Employee.GetDisplaySummary belongs to Employee.cs
and must never be emitted under AuditLogger.cs.
...
# ==============================================================================
# FINAL MASTER ORCHESTRATION PIPELINE
# ==============================================================================
if __name__ == "__main__":
    ...
    # 🚨 STEP 1: HYDRATE LINK GRAPHS TO NEO4J
    section_2_populate_neo4j_graph(JSON_SOURCE, NEO4J_PASSWORD)

    # 🚨 STEP 2: INITIALIZE AI EMBEDDINGS AGENT STACK & HYDRATE VECTORS
    local_llm = ChatOllama(model="qwen2.5-coder:14b", temperature=0.05)
    ...
    # 🚨 STEP 3: FIRE CONVERSATIONAL EXECUTION
